...
(function () {
  ...
  var I18N = {
    lt: {
      ...
      close: 'Uždaryti', myLocation: 'Mano vieta', locator: 'Lokatorius',
    ...
    en: {
      ...
      close: 'Close', myLocation: 'My location', locator: 'Locator',
  ...
  var App = {
    ...
    template: [
      ...
      '      <button type="button" :class="{active: lang===\'lt\'}" :aria-pressed="lang===\'lt\'" @click="setLang(\'lt\')">LT</button>',
      ...
      '        <button class="stats-strip" v-if="stats" type="button" :aria-expanded="mobileStats" @click="mobileStats=!mobileStats">',
      ...
      '          <div class="charts">',
      '            <div class="tabs">',
      '              <button type="button" :class="{active: statTab===\'band\'}" :aria-pressed="statTab===\'band\'" @click="statTab=\'band\'">{{ t("byBand") }}</button>',
      '              <button type="button" :class="{active: statTab===\'mode\'}" :aria-pressed="statTab===\'mode\'" @click="statTab=\'mode\'">{{ t("byMode") }}</button>',
      '            </div>',
      ...
      '        </div>',
      '        <div class="body" v-if="recent.items.length>=recent.limit"><button class="pill" type="button" @click="moreRecent">{{ t("loadMore") }}</button></div>',
      '      </section>',
      ...
      '          <button class="close" type="button" style="float:right" :aria-label="t(\'close\')" @click="clearSearch">×</button></h3>',
      '        <div class="body">',
      '          <div class="legend">',
      '            <button v-for="m in MODES" :key="m" class="pill" type="button" :class="{active:m===mode}" :aria-pressed="m===mode" @click="searchInMode(m)">{{ m.toUpperCase() }}</button>',
      '          </div>',
      '        </div>',
      '        <div class="list" v-if="search.result && search.result.objects.length">',
      '          <button class="item" type="button" v-for="o in search.result.objects" :key="o.code" @click="selectObject(o.code)">',
      ...
      '        <h3>{{ t("myLocation") }}<button class="close" type="button" style="float:right" :aria-label="t(\'close\')" @click="geo.open=false">×</button></h3>',
      '        <div class="body">',
      ...
      '          <button class="close" type="button" :aria-label="t(\'close\')" @click="closeDetail">×</button>',
      '        </div>',
      '      </div>',
